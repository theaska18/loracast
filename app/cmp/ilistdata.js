//ilistdata
Ext.define('IListData', {
	alias:'widget.ilistdata',
	extend:'Ext.Panel',cls:'i-transparent',minHeight:150,addLine:true,paddingBottom:false,property:{},layout:'fit',margin:true,bodyStyle:'border-color:white !important;',
	initComponent:function(){
		var $this=this,property=this.property,fields=[],database=[];
		this.item=this.items;
		property.type='tableeditor';
		var header=[{xtype:'displayfield',value:'&nbsp;',cls:'x-column-header',width:25,border:false,}];
		var allWidth=true,widthHeader=25,objItem=null;
		for(var i=0,iLen=this.item.length; i<iLen; i++){
			...
			if(objItem.text != undefined && objItem.text != null && objItem.text != ''){
				...
				if(objItem.width !== undefined && hidden==false){item.width=objItem.width;widthHeader+=(objItem.width)}
				header.push(item);
			}
		}
		this.fields=fields;
		...
		this.items=[
			// comPanel,
			{xtype:'panel',padding:false,bodyPadding:false,margin:false,bodyMargin:false,layout:'column',id:$this.id+'_listItem',style:'margin-top:-1px;margin-left:-1px;margin-bottom:-1px;margin-right:-1px;',paddingBottom:false,border:false,flex:1,items:[],autoScroll:true,
				// listeners:{
					// render: function(p){
		...
		this._add=function(stat){
			...
			itm.push({
				xtype:'button',name:'del',width: 25,margin: "0 -1 0 0",iconCls:'fa fa-times fa-red',line:listItems.length-1,tabIndex: -1,
	...
	setVal:function(arr){
		...
		if(arr!= undefined){
			if(arr.length==0){
				this.listItem.removeAll();this._addAddButton();
			}
			var $this=this,item=$this.listItem.items.items;
			for(var i=0,iLen=arr.length;i<iLen; i++){
				if(i !=0 || this.addLine==false){$this._add();}
	...
	val:function(arr,arr2){
		...
		for(var j=0,jLen=fields.length; j<jLen; j++){
			var base=database[j] ;
			if(base != undefined && arr2[base.table]== undefined){arr2[base.table]=[];}
			arr[fields[j]+'[]']=[];
		}
	...
	_addAddButton:function(){
		...
		listItem.add({xtype:'panel',cls:'i-transparent',anchor:'100%',style:'padding-top:2px;padding-bottom:4px;',border:false,items:[{
			xtype:'button',
			width: 25,margin: "0 0 0 0",iconCls:'fa fa-plus fa-green',
