...
function initUpdate(){
	$pid=_get('i');
	$ori= _this()->query->row("SELECT distributor_name AS f2,M.distributor_type AS f3,
		...
		M.kelurahan AS f11, M.permission_flag AS f12, M.active_flag AS f13,M.telepon AS f15, M.fax AS f16,M.id_distributor AS f17 FROM inv_distributor M WHERE distributor_id=".$pid);
	if($ori){
		$data=array();
		$data['o']=$ori;
		_data($data);
...
function getList(){
	...
	if($id_distributor != null && $id_distributor !='')
		$criteria.=" AND upper(id_distributor) like upper('%".$id_distributor."%')";
	...
	$res=_this()->query->result("SELECT distributor_id AS i,distributor_code AS f1,distributor_name AS f2,A.distributor_type_name AS f3,
...
function save(){
	$pageType=_post('p');
	$pid=_post('i');
	$distributor_code=_post('f1');
	$distributor_name=_post('f2');
	$type=_post('f3');
	$pic=_post('f4');
	$email=_post('f5');
	$telepon=_post('f15');
	...
	if($pageType=='ADD'){
		...
		if($distributor_code==''){
			$a=false;
			$sequenceCode=getSetting('DISTRIBUTOR','SEQUENCE_CODE');
			while($a==false){
				$codenya=_getSequenceById($sequenceCode);
				$res=_this()->query->row("SELECT distributor_id FROM inv_distributor WHERE distributor_code='".$codenya."' AND tenant_id="._session()->tenant_id);
				if(!$res){
					$distributor_code=$codenya;
					$a=true;
				}
			}
		}else{
			$res=_this()->query->row("SELECT distributor_id FROM inv_distributor WHERE distributor_code='".$distributor_code."' AND tenant_id="._session()->tenant_id);
			if ($res){
				$allow=false;
			}
		}
		if ($allow){
			$id=_getTableSequence('inv_distributor');
			 _this()->query->set("INSERT INTO inv_distributor(distributor_id,distributor_code,distributor_name,
				distributor_type,no_pak,pic,email,address,country,province,city,districts,kelurahan,permission_flag,
				active_flag,create_on,create_by,tenant_id,telepon,fax,id_distributor)VALUES
				(".$id.",'".$distributor_code."','".$distributor_name."',".$type.",'".$pak."','".$pic."','".$email."','".$address."','".$country."','".$province."','".$city."','".$district."','".$districts."',".$permission_flag.",
				".$activeFlag.",'"._format()."',"._session()->employee_id.","._session()->tenant_id.",'".$telepon."','".$fax."','".$id_distributor."')");
			_data(array('i'=>$id,'f1'=>$distributor_code));
			_this()->query->end();
			_message_save('Kode Vendor', $distributor_code );
		}else{
			_this()->query->back();
			_message_exist ('Kode Vendor', $distributor_code );
		}
	}else if($pageType=='UPDATE'){
		 _this()->query->set("UPDATE inv_distributor SET 
			...
			update_by="._session()->employee_id."
				WHERE distributor_id=".$pid);
		_data(array('i'=>$id,'f1'=>$distributor_code));
		_this()->query->end();
		_message_update ('Kode vendor', $distributor_code );
	}
...
function toExcel(){
	...
	$direction='ASC';
	$orderBy.='distributor_name '.$direction;
	$res=_this()->query->result("SELECT distributor_name,A.distributor_type_name,M.no_pak,M.pic,
	...
	$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A1', 'No.')
		->setCellValue('B1', 'Jenis Upline')
		->setCellValue('C1', 'Nama Perusahaan/Pabrik (Asal Produk)')
		->setCellValue('D1', 'Nomor Sertifikat Produksi / Izin PAK')
		->setCellValue('E1', 'PIC/Nama Penanggung Jawab (Asal Produk)')
		->setCellValue('F1', 'Email (Perusahaan Asal Produk)')
		...
		for($i=0,$iLen=count($res);$i<$iLen;$i++){
			...
			$ex->setCellValue('B'.($i+2), $obj->distributor_type_name);
			$ex->setCellValue('C'.($i+2), $obj->distributor_name);
		...
		$objPHPExcel->getProperties()->setCreator(_this()->pagesession->get()->user_code)
            ...
            ->setTitle("Export Distributor")
            ->setSubject("Export Distributor")
            ->setDescription("Export Distributor")
            ->setKeywords("Export Distributor")
            ->setCategory("Export Distributor");
        $objPHPExcel->getActiveSheet()->setTitle('Export Distributor');
        ...
        header('Content-Disposition: attachment;filename="ExportDataDistributor'. date('Ymd') .'.xlsx"');
...
function delete(){
	$pid= _post('i');
	$res=  _this()->query->row("SELECT distributor_code FROM inv_distributor WHERE distributor_id=".$pid);
	if ($res) {
		 _this()->query->set("DELETE FROM inv_distributor WHERE distributor_id=".$pid);
		_message_delete('Kode Vendor', $pid );
	}else
		_not_found();
}