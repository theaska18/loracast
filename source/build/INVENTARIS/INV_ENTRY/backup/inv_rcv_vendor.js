	...
	items : [
		{
						...
						Ext.Ajax.request({
							...
							success : function(response) {
								...
								if (r.r == 'S') {
									...
									table.count();
									Ext.getCmp('INV_RCV_VENDOR.input').closing = false;
									Ext.getCmp('INV_RCV_VENDOR.input').show();
									Ext.getCmp('INV_RCV_VENDOR.input').setTitle('Penerimaan Vendor');
			...
			columns:[
				...
				{ text: 'Tgl. Terima',width: 100,dataIndex: 'f2'},
				{ text: 'Jatuh Tempo',width: 100,dataIndex: 'f3'},
			...
			tbar: [
			{
					...
					if(req == false){
						...
						for(var i=0,iLen=table._getTotal();i<iLen;i++){
							...
							if(table._get('fraction',i)._getValue()==0){
								Ext.create('App.cmp.Toast').toast({msg : 'Faction Tidak Boleh 0.',type : 'warning'});
							...
							if(objCek[table._get('barang',i).getValue()][table._get('sat',i).getValue()] != undefined && objCek[table._get('barang',i).getValue()][table._get('batch',i).getValue()] != undefined){
								error=true;
								Ext.create('App.cmp.Toast').toast({msg : 'Satuan Barang Batch Tidak Boleh Sama.',type : 'warning'});
								table._get('batch',i).focus();
								return false;
							}else{
								objCek[table._get('barang',i).getValue()][table._get('sat',i).getValue()] =true;
								objCek[table._get('barang',i).getValue()][table._get('batch',i).getValue()] =true;
							}
				...
				iconCls:'fa fa-close',
				handler: function() {
					Ext.getCmp('INV_RCV_VENDOR.input').close();
				}
			}],
			items:[
				{
							...
							items:[
								{
									...
									button:{
										...
										items:[
											{
												...
												xtype:'idropdown',
												id : 'INV_RCV_VENDOR.input.f4.f12',
												parameter:'ACTIVE_FLAG',
												name : 'f12',
										...
										columns:[
											...
											{ text: 'Vendor',width: 150, dataIndex: 'name'  },
											{ text: 'Jenis Vendor',width: 200,dataIndex: 'type' },
							...
							items:[
								{
									...
									tabIndex:-1,
									handler:function(a){
										_access('INV_RCV_VENDOR_print_sj',function(){
											if(Ext.getCmp('INV_RCV_VENDOR.input.tableOption')._get('gin',a.line).getValue()!==''){
												...
												Ext.create('App.cmp.Toast').toast({msg : 'Gin Tidak Tersedia.',type : 'warning'});
											}
										})
									}
								},{
