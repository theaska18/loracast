...
function printBarcode(){
	$i=_post('i');
	$dist=_this()->query->row("SELECT G.batch,G.expire_flag,G.expire_date,CONCAT(I.item_code,' - ',I.item_name) AS item FROM inv_gin G 
	INNER JOIN inv_item I ON I.item_id=G.item_id
	WHERE G.gin_code='".$i."'");
	$file=getSetting('INV_RCV_VENDOR','LABEL_URL_REPORT');
	$show=getSetting('INV_RCV_VENDOR','LABEL_PREVIEW');
...
function getListItem(){
	...
	$res=_this()->query->result("SELECT item_id AS i,CONCAT(item_code,' - ',item_name) as name,TYPE.option_name AS type,
		...
		(SELECT S.stock FROM inv_stock S WHERE S.tenant_id=M.tenant_id AND S.item_id=M.item_id AND S.unit_id=".getSetting('INV_RCV_VENDOR','UNIT_ID').") AS stock,
...
function getListVendor(){
	...
	$res=_this()->query->result("SELECT distributor_id AS i,CONCAT(distributor_code,' - ',distributor_name) as name,A.distributor_type_name AS type,
...
function getInitItemByBarcode(){
	...
	$ori= _this()->query->row("SELECT M.item_id AS i,CONCAT(M.item_code,' - ',M.item_name) AS item,M.measurement_buy AS f1,BUY.fraction AS f2,SMALL_MEA.measurement_name AS f3,M.expire_alert AS f4,SMALL_MEA.measurement_id AS f5,
		...
		INNER JOIN inv_item_measurement SELL ON SELL.item_id=M.item_id AND SELL.sell_flag=1
		INNER JOIN inv_measurement SMALL_MEA ON SMALL_MEA.measurement_id=M.measurement_small
		WHERE M.tenant_id="._this()->pagesession->get()->tenant_id." AND M.barcode=".$pid);
	if($ori){
		$data=array();
		$data['o']=$ori;
		_data($data);
...
function initUpdate(){
	$pid=_get('i');
	$ori= _this()->query->row("SELECT receive_on,due_date,posted,M.distributor_id,CONCAT(DIS.distributor_code,' - ',DIS.distributor_name) AS vendor,materai,description
		FROM inv_receive M
		INNER JOIN inv_distributor DIS ON DIS.distributor_id=M.distributor_id
		WHERE receive_id=".$pid);
	$oriList= _this()->query->result("SELECT receive_detail_id,ITM.item_id,CONCAT(ITM.item_code,' - ',ITM.item_name) AS item_name,M.measurement_id,M.qty,M.fraction,
...
function getList(){
	...
	$inner='
		...
		LEFT JOIN inv_item I ON I.item_id=DET.item_id
			';
	$criteria.=" ";
	$criteria.=" M.unit_id=".getSetting('INV_RCV_VENDOR','UNIT_ID')." AND M.tenant_id="._this()->pagesession->get()->tenant_id;
...
function save(){
	...
	if($no_terima==null || $no_terima==''){
		...
		$codenya=_this()->lib_sequence->get('GEN_INV_RECEIVE');
		$no_terima=$codenya['val'];
		_load('lib/lib_table_sequence');
		$pid=_this()->lib_table_sequence->get('inv_receive');
		$add=true;
		
		$par['receive_id']=$pid;
		$par['tenant_id']=_this()->pagesession->get()->tenant_id;
		$par['unit_id']=(double)getSetting('INV_RCV_VENDOR','UNIT_ID');
	...
	for($j=0,$jLen=count($detail_id); $j<$jLen;$j++){
		if($detail_id[$j] ==null || $detail_id =='' ){
			$par=array();
			_load('lib/lib_table_sequence');
			$par['receive_detail_id']=_this()->lib_table_sequence->get('inv_receive_detail');
...
function posting(){
	...
	$res= _this()->query->result("SELECT receive_detail_id,M.item_id,M.measurement_id,(M.qty*M.fraction) AS qty,M.fraction,
		M.general_price,M.expire_flag,M.expire_date,M1.receive_on,M1.unit_id,M.batch,IFNULL(G.qty,0) AS qty_gin,G.gin_id,G.in_date
		FROM inv_receive_detail M 
	...
	for($i=0,$iLen=count($res);$i<$iLen;$i++){
		...
		if(getSetting('INV_RCV_VENDOR','METODE')=='Y'){
			...
			if($o->gin_id == null || $o->gin_id==''){
				...
				$arr['gin_id']=_this()->lib_table_sequence->get('inv_gin');
				$codenya=_this()->lib_sequence->get('GEN_INV_GIN');
				
				$gin_code=$codenya['val'];
				$arrGinCode[]=$gin_code;
			...
			$ginA= _this()->query->row("SELECT M.qty,M.gin_id
				FROM inv_gin M 
		...
		if($row){
			...
			$arr=array();
			$arr['stock_id']=_this()->lib_table_sequence->get('inv_stock');
			$arr['tenant_id']=$tenant_id;
			$arr['item_id']=$o->item_id;
			$arr['unit_id']=$o->unit_id;
...
function unposting(){
	try{
		...
		_this()->db->where('receive_id',$pid);
		_this()->db->update('inv_receive',$par);
		$res= _this()->query->result("SELECT M.gin_id,M1.qty AS qty_gin,M2.item_name,(M.qty*M.fraction) AS qty_terima FROM inv_receive_detail M 
			INNER JOIN inv_gin M1 ON M1.gin_id=M.gin_id 
			INNER JOIN inv_item M2 ON M.item_id=M2.item_id WHERE M.receive_id=".$pid);
		for($i=0,$iLen=count($res);$i<$iLen;$i++){
			if(($res[$i]->qty_gin-$res[$i]->qty_terima)<0){
				_this()->query->back();
				_error_message("Barang '".$res[$i]->item_name."' Sudah Terpakai, Tidak Bisa di Unposting.")->end();
			}
			_this()->query->set("UPDATE inv_gin SET qty=".($res[$i]->qty_gin-$res[$i]->qty_terima)." WHERE gin_id=".$res[$i]->gin_id);
		}
