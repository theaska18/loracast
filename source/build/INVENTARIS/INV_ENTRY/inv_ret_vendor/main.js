	...
	list:[
		{
			key:'f5',
			fn:function(){
				Ext.getCmp('INV_RET_VENDOR.list').refresh();
			}
		},{
			key:'ctrl+f',
			fn:function(){
				Ext.getCmp('INV_RET_VENDOR.text').focus();
			}
		},{
			key:'ctrl+shift+f',
			fn:function(){
				_click('INV_RET_VENDOR.btnShowSearch');
			}
		},{
			key:'f6',
			fn:function(){
				_click('INV_RET_VENDOR.btnAdd');
			}
		}
	]
});
	...
	items : [
		{
						...
						list:[
							{
								key:'esc',
								fn:function(){
									_click('INV_RET_VENDOR.search.btnSearch');
								}
							},{
								key:'ctrl+r',
								fn:function(){
									_click('INV_RET_VENDOR.search.btnReset');
								}
							},{
								key:'ctrl+s',
								fn:function(){
								...
								key:'ctrl+f',
								fn:function(){
									Ext.getCmp('INV_RET_VENDOR.search.f1').focus();
								}
							}
						]
					});
			...
			fbar: [
				{
					...
					iconCls:'fa fa-search',
					id:'INV_RET_VENDOR.search.btnSearch',
					handler: function() {
						Ext.getCmp('INV_RET_VENDOR.list').refresh();
					}
				},{
					...
					id:'INV_RET_VENDOR.search.btnReset',
					handler: function() {
						Ext.getCmp('INV_RET_VENDOR.search.panel').qReset();
					}
				},{
					text: 'Keluar',
					...
					iconCls:'fa fa-close',
					handler: function() {
						Ext.getCmp('INV_RET_VENDOR.search').close();
					}
				}
			],
			items:[
				{
					xtype:'ipanel',
					id : 'INV_RET_VENDOR.search.panel',
					width: 350,
					items:[
						{
							xtype:'itextfield',
							...
							id:'INV_RET_VENDOR.search.f6'
						},{ 
							xtype:'idropdown',
							id : 'INV_RET_VENDOR.search.f8',
			...
			xtype:'itable',
			id:'INV_RET_VENDOR.list',
			params:function(bo){
				if(bo==true){
					var arr=Ext.getCmp('INV_RET_VENDOR.search.panel').qParams();
					arr['unit_id']=getSetting('INV_RET_VENDOR','UNIT_ID');
					return arr;
				}else{
					var obj={};
					obj[Ext.getCmp('INV_RET_VENDOR.dropdown').getValue()]=Ext.getCmp('INV_RET_VENDOR.text').getValue();
					obj['unit_id']=getSetting('INV_RET_VENDOR','UNIT_ID');
					return obj;
				}
			},
			url:url + 'cmd?m=INV_RET_VENDOR&f=getList',
			result:function(response){
				return {list:response.d,total:response.t};
			},
			fn:{
				delete:function(a){
					_access('INV_RET_VENDOR_DELETE',function(){
						Ext.getCmp('INV_RET_VENDOR.confirm').confirm({
							msg : "Apakah akan menghapus No. Retur '"+a.f1+"'",
							allow : 'INV_RET_VENDOR.delete',
							onY : function() {
								Ext.Ajax.request({
									url : url + 'cmd?m=INV_RET_VENDOR&f=delete',
									method : 'POST',
									params : {
										i : a.i
									},
									before:function(){
										Ext.getCmp('INV_RET_VENDOR.list').setLoading(true);
									},
									success : function(response) {
										Ext.getCmp('INV_RET_VENDOR.list').setLoading(false);
				...
				update:function(a){
					_access('INV_RET_VENDOR_UPDATE',function(){
						Ext.Ajax.request({
							url : url + 'cmd?m=INV_RET_VENDOR&f=initUpdate',
							params:{i:a.i},
							method : 'GET',
							before:function(){
								Ext.getCmp('INV_RET_VENDOR.list').setLoading(true);
							...
							success : function(response) {
								...
								var r = ajaxSuccess(response);
								if (r.r == 'S') {
									var o=r.d.o,l=r.d.l,table=Ext.getCmp('INV_RET_VENDOR.input.tableOption');
									Ext.getCmp('INV_RET_VENDOR.input.panel').qReset();
									table.resetTable();
									...
									Ext.getCmp('INV_RET_VENDOR.input.i').setValue(a.i);
									Ext.getCmp('INV_RET_VENDOR.input.f1').setValue(a.f1);
									Ext.getCmp('INV_RET_VENDOR.input.f2').setValue(o.retur_on);
									Ext.getCmp('INV_RET_VENDOR.input.f3').setValue(o.description);
									if(o.posted ==0){
										Ext.getCmp('INV_RET_VENDOR.input.btnPosting').setIconCls('fa fa-check');
										Ext.getCmp('INV_RET_VENDOR.input.btnPosting').setText('Posting');
										Ext.getCmp('INV_RET_VENDOR.input.f2').setReadOnly(false);
									}else{
										Ext.getCmp('INV_RET_VENDOR.input.btnPosting').setIconCls('fa fa-close');
										Ext.getCmp('INV_RET_VENDOR.input.btnPosting').setText('Unposting');
										Ext.getCmp('INV_RET_VENDOR.input.f2').setReadOnly(true);
									}
									table.count();
									Ext.getCmp('INV_RET_VENDOR.input').closing = false;
									Ext.getCmp('INV_RET_VENDOR.input').show();
									Ext.getCmp('INV_RET_VENDOR.input').setTitle('Retur Vendor');
									Ext.getCmp('INV_RET_VENDOR.input.f2').focus();
									Ext.getCmp('INV_RET_VENDOR.input.panel').qSetForm();
								}
							},
							failure : function(jqXHR, exception) {
								Ext.getCmp('INV_RET_VENDOR.list').setLoading(false);
			...
			press:{
				delete:function(a){
					Ext.getCmp('INV_RET_VENDOR.list').fn.delete(a.dataRow);
				},
				enter:function(a){
					Ext.getCmp('INV_RET_VENDOR.list').fn.update(a.dataRow);
				}
			},
			tbar:[{xtype:'ibuttonnewtab'},{xtype:'ibuttonfullscreen'},'-',
			{
				xtype:'iconfig',
				id:'INV_RET_VENDOR.config',
				code:[
					iif(_access('INV_RET_VENDOR_config_UNIT_ID')==false,'UNIT_ID',null)
				]
			},
			'-','->','-',{
				text: 'Tambah',
				tooltip:'Tambah <b>[F6]</b>',
				id:'INV_RET_VENDOR.btnAdd',
				iconCls: 'fa fa-plus',
				handler:function(a){
					var table=Ext.getCmp('INV_RET_VENDOR.input.tableOption');
					...
					Ext.getCmp('INV_RET_VENDOR.input.f2').setReadOnly(false);
					Ext.getCmp('INV_RET_VENDOR.input').show();
					Ext.getCmp('INV_RET_VENDOR.input').setTitle('Retur');
					Ext.getCmp('INV_RET_VENDOR.input.f2').focus();
					Ext.getCmp('INV_RET_VENDOR.input.panel').qSetForm();
				}
			},{
					xtype:'idropdown',
					id : 'INV_RET_VENDOR.dropdown',
					emptyText:'Pencarian',
					margin:false,
					value:'f1',
					data:[
					...
					press:{
						enter:function(){
							_click('INV_RET_VENDOR.btnSearch');
						}
					}
				},{
					xtype:'itextfield',
					...
					id:'INV_RET_VENDOR.btnSearch',
					handler : function(a) {
						Ext.getCmp('INV_RET_VENDOR.list').refresh(false);
					}
				},'-',{
					text: 'Pencarian',
					tooltip:'Pencarian <b>[Ctrl+Shift+f]</b>',
					id:'INV_RET_VENDOR.btnShowSearch',
					iconCls: 'fa fa-search',
					handler:function(a){
						Ext.getCmp('INV_RET_VENDOR.search').show();
						Ext.getCmp('INV_RET_VENDOR.search.f1').focus();
					}
				}
			],
			...
			viewConfig: {
                listeners: {
                    expandbody: function(rowNode, record, expandNode) {
                        ...
                        theTd.mask('Loading...');
						theTd.update('<iframe src="'+url + 'cmd?m=INV_RET_VENDOR&f=getDetail&session='+_session_id+'&i='+record.data.i+'" style="width: 100%;" scrolling="no" onload="resizeIframe(this)"></iframe>');
                        theTd.unmask();
					}
				}
			},
			columns:[
				...
				{ text: 'Posting',width: 50,sortable :false,dataIndex: 'f5',align:'center',
					...
					handler: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
						Ext.getCmp('INV_RET_VENDOR.list').fn.update(record.data);
					}
				},{
					text: 'Hapus',
					xtype: 'actioncolumn',
					iconCls: 'fa fa-trash',
					handler: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
						Ext.getCmp('INV_RET_VENDOR.list').fn.delete(record.data);
					}
				}
						...
						list:[
							{
								key:'ctrl+s',
								fn:function(){
									_click('INV_RET_VENDOR.input.btnSave');
								}
							},{
								key:'esc',
								fn:function(){
									_click('INV_RET_VENDOR.input.btnClose');
								}
							},{
								key:'f6',
								fn:function(){
									_click('INV_RET_VENDOR.input.btnNew');
								}
							},{
								key:'f7',
								fn:function(){
									_click('INV_RET_VENDOR.input.btnPosting');
								}
							},{
								key:'f8',
								fn:function(){
									_click('INV_RET_VENDOR.input.btnPay');
								}
							}
						]
					});
			...
			tbar: [
			{
				text: 'New',
				tooltip:'New <b>[F6]</b>',
				id:'INV_RET_VENDOR.input.btnNew',
				iconCls:'fa fa-file',
				handler: function() {
					var $this = this;
					$this.closing = false;
					if (Ext.getCmp('INV_RET_VENDOR.input.panel').qGetForm() == false)
						Ext.getCmp('INV_RET_VENDOR.confirm').confirm({
							msg :'Apakah akan mengabaikan data yang sudah diubah?',
							allow : 'INV_RET_VENDOR.close',
							onY : function() {
								var table=Ext.getCmp('INV_RET_VENDOR.input.tableOption');
								table._getAddButton().enable();
								Ext.getCmp('INV_RET_VENDOR.input.panel').qReset();
						...
						Ext.getCmp('INV_RET_VENDOR.input.panel').qReset();
						Ext.getCmp('INV_RET_VENDOR.input.tableOption').resetTable();
						Ext.getCmp('INV_RET_VENDOR.input.btnPosting').setIconCls('fa fa-check');
						Ext.getCmp('INV_RET_VENDOR.input.btnPosting').setText('Posting');
						Ext.getCmp('INV_RET_VENDOR.input.f2').setReadOnly(false);
				...
				id:'INV_RET_VENDOR.input.btnSave',
				iconCls:'fa fa-save',
				handler: function() {
					var req=Ext.getCmp('INV_RET_VENDOR.input.panel').qGetForm(true);
					if(req == false){
						var error=false;
						var table=Ext.getCmp('INV_RET_VENDOR.input.tableOption');
						var objCek={};
						...
						Ext.getCmp('INV_RET_VENDOR.confirm').confirm({
							...
							allow : 'INV_RET_VENDOR.save',
							onY : function() {
								var param = Ext.getCmp('INV_RET_VENDOR.input.panel').qParams();
								param['unit_id']=getSetting('INV_RET_VENDOR','UNIT_ID');
								Ext.Ajax.request({
									url : url + 'cmd?m=INV_RET_VENDOR&f=save',
									method : 'POST',
									params:param,
									before:function(){
										Ext.getCmp('INV_RET_VENDOR.input').setLoading(true);
									},
									success : function(response) {
										Ext.getCmp('INV_RET_VENDOR.input').setLoading(false);
										var r = ajaxSuccess(response);
										if (r.r == 'S') {
											...
											Ext.getCmp('INV_RET_VENDOR.input.i').setValue(r.d.id);
											Ext.getCmp('INV_RET_VENDOR.input.f1').setValue(r.d.code);
											Ext.getCmp('INV_RET_VENDOR.list').refresh();
											Ext.getCmp('INV_RET_VENDOR.input.f2').focus();
											Ext.getCmp('INV_RET_VENDOR.input.panel').qSetForm();
										}
						...
						Ext.getCmp('INV_RET_VENDOR.input').qClose();
				}
			},'-',{
				text: 'Posting',
				tooltip:'Posting/UnPosting <b>[F7]</b>',
				id:'INV_RET_VENDOR.input.btnPosting',
				iconCls:'fa fa-check',
				handler: function(a) {
					var table=Ext.getCmp('INV_RET_VENDOR.input.tableOption');
					if(a.getText()=='Posting'){
						_access('INV_RET_VENDOR_posting',function(){
							if(Ext.getCmp('INV_RET_VENDOR.input.c').getValue()==''){
								Ext.getCmp('INV_RET_VENDOR.input.c').setValue('Y');
							}else{
								Ext.getCmp('INV_RET_VENDOR.input.c').setValue('');
							}
							var req=Ext.getCmp('INV_RET_VENDOR.input.panel').qGetForm(true);
							if(req == false){
								var error=false;
								...
								Ext.getCmp('INV_RET_VENDOR.confirm').confirm({
									...
									allow : 'INV_RET_VENDOR.posting',
									onY : function() {
										var param = Ext.getCmp('INV_RET_VENDOR.input.panel').qParams();
										param['unit_id']=getSetting('INV_RET_VENDOR','UNIT_ID');
										Ext.Ajax.request({
											url : url + 'cmd?m=INV_RET_VENDOR&f=posting',
											method : 'POST',
											params:param,
											before:function(){
												Ext.getCmp('INV_RET_VENDOR.input').setLoading(true);
						...
						_access('INV_RET_VENDOR_unposting',function(){
							Ext.getCmp('INV_RET_VENDOR.confirm').confirm({
								msg : 'Apakah akan di UnPosting?',
								allow : 'INV_RET_VENDOR.posting',
								onY : function() {
									Ext.Ajax.request({
										url : url + 'cmd?m=INV_RET_VENDOR&f=unposting',
										method : 'POST',
										params:{i:Ext.getCmp('INV_RET_VENDOR.input.i').getValue()},
										before:function(){
											Ext.getCmp('INV_RET_VENDOR.input').setLoading(true);
										},
										success : function(response) {
					...
					items:[
						{
							...
							items:[
								{
									xtype:'ihiddenfield',
									name:'i',
									id:'INV_RET_VENDOR.input.i'
								},{
									xtype:'ihiddenfield',
									name:'c',
									id:'INV_RET_VENDOR.input.c'
								},{
									xtype:'itextfield',
									id : 'INV_RET_VENDOR.input.f1',
									labelAlign:'top',
									width: 100,
									readOnly:true,
									name : 'f1',
							...
							xtype:'ilistinput',
							id:'INV_RET_VENDOR.input.tableOption',
							name:'options',
							flex:3,
							...
							count:function(){
								...
								Ext.getCmp('INV_RET_VENDOR.input.total')._setValue(jumlahHarga);
							},
							items:[
								{
									xtype:'iselect',
									...
									onSelect:function(a,b){
										...
										if(table.loadQuery==true){
											Ext.Ajax.request({
												url : url + 'cmd?m=INV_RET_VENDOR&f=getInitItem',
												method : 'GET',
												params : {
													i : a.i
												},
												success : function(response) {
													var r = ajaxSuccess(response);
													if (r.r == 'S'){
														var o=r.d.o,table=Ext.getCmp('INV_RET_VENDOR.input.tableOption');
														if(a.harga==null){
															a.harga=0;
														}
										...
										Ext.getCmp('INV_RET_VENDOR.input.item.unit_id').setValue(getSetting('INV_RET_VENDOR','UNIT_ID'));
									},
									button:{
										urlData : url + 'cmd?m=INV_RET_VENDOR&f=getListItem',
										windowWidth: 800,
										items:[
											{
												xtype:'itextfield',
												...
												xtype:'ihiddenfield',
												name:'unit_id',
												id:'INV_RET_VENDOR.input.item.unit_id',
											},{
												xtype:'itextfield',
												name:'f1',
		...
		},{xtype:'iconfirm',id : 'INV_RET_VENDOR.confirm'}
	]
});