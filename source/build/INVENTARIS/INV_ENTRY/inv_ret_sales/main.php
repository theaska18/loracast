...
function getListItem(){
	...
	$now=new DateTime();
		...
		$inner='	INNER JOIN inv_item ITEM ON ITEM.item_id=M.item_id
					...
					INNER JOIN inv_trans_partners_detail RECD ON RECD.gin_id=M.gin_id
					INNER JOIN inv_receive_detail REC2D ON REC2D.gin_id=M.gin_id
					INNER JOIN inv_trans_partners REC ON REC.trans_id=RECD.trans_id
		...
		$criteria.=" AND RECD.qty>0 AND M.unit_id=".$unit_id." ";
		if($code != null && $code !='')
		...
		if($no_terima != null && $no_terima !='')
			$criteria.=" AND upper(REC.trans_code) like upper('%".$no_terima."%')";
		if($desc != null && $desc !='')
		...
		$res=_this()->query->result("SELECT ITEM.item_id AS i,RECD.trans_detail_id AS gin_id,CONCAT(ITEM.item_code,' - ',ITEM.item_name) as name,
			RECD.qty/RECD.fraction AS stock,IM.fraction AS frac
			,ITEM.active_flag AS a,SAT_S.measurement_name AS sat_s,REC2D.batch,
			M.in_date AS tgl,M.gin_code AS gin,REC.trans_code AS terima,RECD.price/RECD.fraction AS harga,
...
function initUpdate(){
	...
	$ori= _this()->query->row("SELECT M.retur_on,M.posted,M.retur_number,
		description
		FROM inv_retur_trans_partners M
		WHERE retur_trans_partners_id=".$pid);

	$oriList= _this()->query->result("SELECT 
		M.trans_detail_id AS gin_id,GIN.gin_code,M.retur_trans_partners_detail_id AS retur_receive_detail_id,ITM.item_id,
		CONCAT(ITM.item_code,' - ',ITM.item_name) AS item_name,
		...
		M.price,CASE WHEN M.fraction=0 THEN 1 ELSE M.fraction END AS fraction,M.description,(D.price/D.fraction) AS buy_price,
		(GIN.qty-GIN.qty_out)+CASE WHEN P.posted=1 THEN M.qty ELSE 0 END AS sisa
		FROM inv_retur_trans_partners_detail M
		INNER JOIN inv_retur_trans_partners P ON P.retur_trans_partners_id=M.retur_trans_partners_id
		INNER JOIN inv_trans_partners_detail M2 ON M2.trans_detail_id=M.trans_detail_id
		INNER JOIN inv_gin GIN ON GIN.gin_id=M2.gin_id
		INNER JOIN inv_trans_partners_detail D ON D.gin_id=GIN.gin_id
		INNER JOIN inv_item ITM ON ITM.item_id=GIN.item_id
		INNER JOIN inv_measurement SAT ON SAT.measurement_id=M.measurement_id
		
		WHERE M.retur_trans_partners_id=".$pid." ORDER BY M.line ASC");
	if($ori){
		$data=array();
		$data['o']=$ori;
		$data['l']=$oriList;
...
function getList(){
	...
	$unit_id=_get('unit_id');
	$entity=' inv_retur_trans_partners ';
	$criteria=" WHERE";
	$inner='
			INNER JOIN inv_retur_trans_partners_detail DE ON DE.retur_trans_partners_id=M.retur_trans_partners_id
			INNER JOIN inv_trans_partners_detail D ON D.trans_detail_id=DE.trans_detail_id
			INNER JOIN inv_trans_partners P ON P.trans_id=D.trans_id
			INNER JOIN inv_partners PART ON PART.partners_id=P.partners_id';
	$criteria.=" ";
	...
	if($noTerima != null && $noTerima !='')
		$criteria.=" AND upper(P.trans_code) like upper('%".$noTerima."%')";
	...
	if($namaVendor != null && $namaVendor !=''){
		$criteria.=" AND upper(PART.parters_name) like upper('%".$namaVendor."%')";
	}
	$orderBy=' ORDER BY ';
	if($direction == null)
		$direction='DESC';
	...
	$total=_this()->query->row("SELECT COUNT(X.retur_trans_partners_id) AS total FROM(SELECT M.retur_trans_partners_id FROM ".$entity." M  ".$inner." ".$criteria. " GROUP BY M.retur_trans_partners_id)X ");
	$res=_this()->query->result("SELECT M.retur_trans_partners_id AS i,retur_number AS f1,retur_on AS f2,M.description AS f3,M.posted AS f5 FROM ".$entity." M ".$inner." ".$criteria." GROUP BY M.retur_trans_partners_id ".$orderBy.' LIMIT '.$size.' OFFSET '.$first);
	_data($res)->setTotal($total->total);
}
function save(){
	_this()->query->start();
	...
	if($no_jual==null || $no_jual==''){
		$codenya=_this()->lib_sequence->get('NO_RETUR_TRANS_PARTNERS');
		$no_jual=$codenya['val'];
		$pid=_this()->lib_table_sequence->get('inv_retur_trans_partners');
		$add=true;
		$par['retur_trans_partners_id']=$pid;
	...
	if($add==true){
		...
		_this()->db->insert('inv_retur_trans_partners',$par);
	}else{
		$par['update_by']=_this()->pagesession->get()->employee_id;
		$par['update_on']=$now->format('Y-m-d H:i:s');
		$post= _this()->query->row("SELECT posted FROM inv_retur_trans_partners WHERE retur_trans_partners_id=".$pid);
		if($post->posted==1){
			$posting=true;
		}
		_this()->db->where('retur_trans_partners_id',$pid);
		_this()->db->update('inv_retur_trans_partners',$par);
	}
	$res= _this()->query->result("SELECT retur_trans_partners_detail_id FROM inv_retur_trans_partners_detail WHERE retur_trans_partners_id=".$pid);
	$line=1;
	for($i=0,$iLen=count($res); $i<$iLen;$i++){
		$ada=false;
		for($j=0,$jLen=count($detail_id); $j<$jLen;$j++){
			if($detail_id[$j] !=null && $detail_id !='' ){
				if((int)$res[$i]->retur_trans_partners_detail_id==(int)$detail_id[$j]){
					$ada=true;
					$par=array();
					if($posting==false){
						$par['measurement_id']=$satuan[$j];
						$par['trans_detail_id']=$gin[$j];
					...
					$par['update_by']=_this()->pagesession->get()->employee_id;
					$par['update_on']=$now->format('Y-m-d H:i:s');
					_this()->db->where('retur_trans_partners_detail_id',$detail_id[$j]);
					_this()->db->update('inv_retur_trans_partners_detail',$par);
				}
			}
		}
		if($ada==false){
			_this()->query->set("DELETE FROM inv_retur_trans_partners_detail WHERE retur_trans_partners_detail_id=".$res[$i]->retur_trans_partners_detail_id);
		}
	}
	$arrId=array();
	for($j=0,$jLen=count($detail_id); $j<$jLen;$j++){
		if($detail_id[$j] ==null || $detail_id =='' ){
			$par=array();
			_load('lib/lib_table_sequence');
			$par['retur_trans_partners_detail_id']=_this()->lib_table_sequence->get('inv_retur_trans_partners_detail');
			$arrId[]=$par['retur_trans_partners_detail_id'];
			$par['retur_trans_partners_id']=$pid;
			$par['measurement_id']=$satuan[$j];
			$par['qty']=$qty[$j]*$fraction[$j];
			$par['price']=$harga[$j];
			...
			$par['create_by']=_this()->pagesession->get()->employee_id;
			$par['create_on']=$now->format('Y-m-d H:i:s');
			_this()->db->insert('inv_retur_trans_partners_detail',$par);
		}
	}
	_this()->query->end();
...
function posting(){
	...
	$res= _this()->query->result("SELECT M2.gin_id,M.qty,G.item_id,P.unit_id,ITM.item_name 
		FROM inv_retur_trans_partners_detail M 
		INNER JOIN inv_retur_trans_partners P ON P.retur_trans_partners_id=M.retur_trans_partners_id 
		INNER JOIN inv_trans_partners_detail M2 ON M2.trans_detail_id=M.trans_detail_id 
		INNER JOIN inv_gin G ON G.gin_id=M2.gin_id 
		INNER JOIN inv_item ITM ON ITM.item_id=G.item_id WHERE M.retur_trans_partners_id=".$pid);
	for($i=0,$iLen=count($res);$i<$iLen;$i++){
		$o=$res[$i];
		$arr=array();
...
function unposting(){
	...
	$row=_this()->query->row("SELECT M.posted FROM inv_retur_trans_partners M WHERE M.retur_trans_partners_id=".$pid);
	if($row){
		if($row->posted==0){
			_this()->query->back();
	...
	_this()->db->where('retur_trans_partners_id',$pid);
	_this()->db->update('inv_retur_trans_partners',$par);
	$res= _this()->query->result("SELECT M3.gin_id,M.qty FROM inv_retur_trans_partners_detail M 
		INNER JOIN inv_trans_partners_detail M3 ON M3.trans_detail_id=M.trans_detail_id 
		INNER JOIN inv_gin M1 ON M1.gin_id=M3.gin_id 
		INNER JOIN inv_item M2 ON M1.item_id=M2.item_id WHERE M.retur_trans_partners_id=".$pid);
	for($i=0,$iLen=count($res);$i<$iLen;$i++){
		$obj1= _this()->query->row("SELECT G.qty, G.gin_code FROM inv_gin G WHERE gin_id=".$res[$i]->gin_id);
		if(($obj1->qty-$res[$i]->qty)<0){
			_this()->query->back();
			_error_message("Kode GIN '".$obj1->gin_code."' Minus, tidak dapat di Unposting.")->end();
		}
		$arr['qty']=$obj1->qty-$res[$i]->qty;
		_this()->db->where('gin_id',$res[$i]->gin_id);
		_this()->db->update('inv_gin',$arr);
	}
	$res= _this()->query->result("SELECT G.item_id,M.qty,M1.unit_id,M2.item_name FROM inv_retur_trans_partners_detail M 
		INNER JOIN inv_retur_trans_partners M1 ON M1.retur_trans_partners_id=M.retur_trans_partners_id 
		INNER JOIN inv_trans_partners_detail M3 ON M3.trans_detail_id=M.trans_detail_id 
		INNER JOIN inv_gin G ON G.gin_id=M3.gin_id 
		INNER JOIN inv_item M2 ON M2.item_id=G.item_id WHERE M.retur_trans_partners_id=".$pid);
	for($i=0,$iLen=count($res);$i<$iLen;$i++){
...
function delete(){
	$pid= _post('i');
	$res=  _this()->query->row("SELECT retur_number,posted FROM inv_retur_trans_partners WHERE retur_trans_partners_id=".$pid);
	if ($res) {
		if($res->posted==0){
			 _this()->query->set("DELETE FROM inv_retur_trans_partners WHERE retur_trans_partners_id=".$pid);
