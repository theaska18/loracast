...
function printBarcode(){
	...
	WHERE G.gin_code='".$i."'");
	$file=getSetting('INV_RCV_VENDOR','LABEL_FILE_NAME',false,'File Label Barcode');
	$show=getSetting('INV_RCV_VENDOR','LABEL_PREVIEW',false,'Preview Label');
...
function getListItem(){
	...
	$res=_this()->query->result("SELECT item_id AS i,CONCAT(item_name,' - ',item_code) as name,
		...
		(SELECT S.stock FROM inv_stock S WHERE S.tenant_id=M.tenant_id AND S.item_id=M.item_id AND S.unit_id=".getSetting('INV_RCV_VENDOR','UNIT_ID',false,'Unit').") AS stock,
