<?php
 ...
 require_once dirname(__FILE__) . '/Exception/NoFaceException.php';
class FaceDetector{
    ...
    public function faceDetect($file)
    {
        if (is_resource($file)) {
            ...
            $this->canvas = imagecreatefromstring($decoded);
        } else {
    ...
    {
        if (empty($this->face)) {
			$ci->jsonresult->error()->setMessage('Wajah Tidak Terdeteksi.')->end();
	...
	public function cropFaceToBase64($outFileName = null){
		$ci=&get_instance();
        if (empty($this->face)) {
			$ci->jsonresult->error()->setMessage('Wajah Tidak Terdeteksi.')->end();
            // throw new NoFaceException('No face detected');
