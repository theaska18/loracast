							...
							<div class="skill-thumb">
								 <div class="wow fadeInUp section-title color-white bg-yellow" data-wow-delay="1.2s" style="margin-top: 100px;border-top:1px solid #c26d34;padding: 20px 20px 20px 30px !important;">
									  ...
									  <?php
										...
										?>
											<a alt="<?php echo $app_name; ?>"  href="<?php echo base_url(); ?>article?src=<?php echo str_replace(' ','+',strtolower($resArticle[$i]->title)); ?>"><img src="<?php echo $resArticle[$i]->page_image; ?>&w=100&h=100" style="width: 100px;border: 1px solid #ebebeb;" /></a><br>
										<?php
											}
										?>
										</div>
										<div style="">
											<span style="color:#90949c;"><?php echo waktu_lalu($tglPostTerbaru->format('Y-m-d H:i:s')); ?></span> - <a alt="<?php echo $app_name; ?>"   href="<?php echo base_url(); ?>article?src=<?php echo str_replace(' ','+',strtolower($resArticle[$i]->title)); ?>" title="<?php echo $resArticle[$i]->title; ?>"><u><?php echo $resArticle[$i]->title; ?></u></a>
								 ...
								 <div class="wow fadeInUp section-title color-white" data-wow-delay="1.2s" style="padding: 0px 20px 20px 30px !important;">
									  ...
									  <?php
										...
										<?php
											if($resArticle[$i]->page_image != null && $resArticle[$i]->page_image != ''){
										?>
											<a alt="<?php echo $app_name; ?>"  href="<?php echo base_url(); ?>article?src=<?php echo str_replace(' ','+',strtolower($resArticle[$i]->title)); ?>"><img src="<?php echo $resArticle[$i]->page_image; ?>&w=100&h=100" style="width: 100px;float:left;margin-right: 10px;border: 1px solid #ebebeb;" /></a>
										<?php
											}
										?>
											<a alt="<?php echo $app_name; ?>" href="<?php echo base_url(); ?>article?src=<?php echo str_replace(' ','+',strtolower($resArticle[$i]->title)); ?>" title="<?php echo $resArticle[$i]->title; ?>"><u><?php echo $resArticle[$i]->title; ?></u></a><br>
							...
							<div class="row">
								...
								<hr>
										...
										?>
											...
											<a alt="<?php echo $app_name; ?>"   style="color:blue !important;" href="<?php echo base_url(); ?>article?src=<?php echo str_replace(' ','+',strtolower($resArticle[$i]->title)); ?>" title="<?php echo $resArticle[$i]->title; ?>"><u><?php echo $resArticle[$i]->title; ?></u></a>
							...
							<div class="row">
								<div class="col-md-12"><br>
										<?php
											...
											$countArticel=$ini->query->row("SELECT count(article_id) AS jum
												FROM con_article WHERE active_flag=1 ".$query." AND system_flag=0 AND post_type in('POST_ARTICLE','POST_SHOP','POST_NEWS')")->jum/$perpage;
											$resArticle=$ini->query->result("SELECT page_image,title,create_on,SUBSTRING_INDEX(SUBSTRING_INDEX(text,'<!-- pagebreak -->', 1),'<!-- pagebreak -->', -1) AS text
												FROM con_article WHERE active_flag=1 ".$query." AND system_flag=0 AND post_type in('POST_ARTICLE','POST_SHOP','POST_NEWS') ORDER BY create_on DESC LIMIT ".$perpage." OFFSET ".$offest);
											if(count($resArticle)>0){
												for($i=0,$iLen=count($resArticle); $i<$iLen; $i++){
													$tglPostTerbaru=new DateTime($resArticle[$i]->create_on);
										?>
										<div>
											...
											?>
											<span style="font-weight: bold;"><a alt="<?php echo $app_name; ?>"   style="color:blue !important;" href="<?php echo base_url(); ?>article?src=<?php echo str_replace(' ','+',strtolower($resArticle[$i]->title)); ?>" title="<?php echo $resArticle[$i]->title; ?>"><?php echo $resArticle[$i]->title; ?></a></span> 
										<?php
											$jumComment=$ini->query->row("SELECT count(comment_id) AS jum FROM con_comment WHERE url='".base_url()."article?src=".str_replace(' ','+',strtolower($resArticle[$i]->title))."' AND parent_id is null")->jum;
											if($jumComment>0){
										?>
											· <span style="color:#90949c;font-size: 11px;"><?php echo $jumComment;?> Komentar</span>
										<?php
											}
										?>
